...
> the root directory is two levels above our home directory, so **`cd`** or **`cd ~`** will take you to **`/home/user`**
...
```output
-rw-rw-r-- 1 user user 43332 Nov 15 23:02 SRR098026-backup.fastq
```
...
Our goal for now is to change permissions on this file so that you (and others) no longer have **`w`** or write permissions. We can do this using the **`chmod`** (change mode) command and subtracting **(`-`)** the write permission **`-w`**. 
...
```output
-r--r--r-- 1 user user 43332 Nov 15 23:02 SRR098026-backup.fastq
```
